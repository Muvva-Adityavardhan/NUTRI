...
#         # Core Macros
#         if "name" in c_lower or "food" in c_lower:
#             col_map["name"] = col
#         elif any(x in c_lower for x in ["energy_kcal", "calor", "kcal"]):
#             col_map["calories"] = col
#         elif "protein_g" in c_lower:
#             col_map["protein"] = col
#         elif any(x in c_lower for x in ["total lipid", "fat_g", "fats"]):
#             col_map["fat"] = col
#         elif "carbohydrate" in c_lower or "carb_g" in c_lower:
#             col_map["carbs"] = col
#         elif "cholesterol_mg" in c_lower:
#             col_map["cholesterol"] = col
        
#         # Micronutrients (Vitamins & Minerals) detection
#         elif any(x in c_lower for x in ["vitamin", "vit ", "calcium", "iron", "zinc", "magnesium", "sodium", "potassium"]):
#             micronutrient_cols.append(col)

#     if "name" not in col_map:
#         print("Could not identify a 'Name' column. Check CSV headers.")
#         return {}
...
#             # Remove non-numeric chars like 'g', 'mg' then convert
#             return pd.to_numeric(df[col_name].astype(str).str.replace(r'[^\d.]', '', regex=True), errors='coerce').fillna(0)
#         return 0

#     df["calories"] = clean_numeric("calories")
#     df["protein"] = clean_numeric("protein")
#     df["fat"] = clean_numeric("fat")
#     df["carbs"] = clean_numeric("carbs")
#     df["cholesterol"] = clean_numeric("cholesterol")
...
#         # Gather Micronutrients into a readable string list
#         micros = []
#         for micro_col in micronutrient_cols:
#             val = row[micro_col]
#             if pd.notnull(val) and val != 0 and str(val).strip() != "":
#                 # Format: "Vitamin C: 10mg"
#                 micros.append(f"{micro_col}: {val}")
...
#             "micros": micros[:4] # Limit to top 4 nutrients to save space
#         }
...
#     print(f"Loaded {len(df)} items with Nutrients.")
