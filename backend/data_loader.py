...
# # Multiplier is used to sort "best" vitamins by value (mg * 1000 = ug)
# MICRONUTRIENTS_MAP = {
...
#     # Helper to safely clean numeric columns
...
#         # 2. Main Macros - Prefer 'unit_serving' columns if available, else standard
#         # Calories
...
#         # We collect all valid micronutrients, normalize them to micrograms (ug) for comparison,
#         # sort them, and pick the top 5.
        
#         all_micros = []
#         for col, (disp_name, unit, multiplier) in MICRONUTRIENTS_MAP.items():
#             # Try serving unit column first (e.g., unit_serving_calcium_mg)
#             serving_col = "unit_serving_" + col
            
#             val = clean_numeric(row.get(serving_col, 0))
#             if val == 0: # Fallback to standard column
#                 val = clean_numeric(row.get(col, 0))
            
#             if val > 0:
#                 normalized_val = val * multiplier # Convert to ug for sorting
#                 # Store tuple: (normalized_val, original_val, display_name, unit)
...
#     print(f"Loaded {len(df)} items. Extracted Nutrients & Serving Sizes.")
...
# --- Keywords to identify Non-Veg foods ---
NON_VEG_KEYWORDS = [
    "chicken", "mutton", "fish", "egg", "omelette", "prawn", "shrimp", 
    "beef", "pork", "meat", "lamb", "seafood", "crab", "bacon", 
    "sausage", "ham", "salami"
]
...
MICRONUTRIENTS_MAP = {
    "calcium_mg": ("Calcium", "mg", 1000), "phosphorus_mg": ("Phosphorus", "mg", 1000),
    "magnesium_mg": ("Magnesium", "mg", 1000), "sodium_mg": ("Sodium", "mg", 1000),
    "potassium_mg": ("Potassium", "mg", 1000), "iron_mg": ("Iron", "mg", 1000),
    "copper_mg": ("Copper", "mg", 1000), "selenium_ug": ("Selenium", "ug", 1),
    "chromium_mg": ("Chromium", "mg", 1000), "manganese_mg": ("Manganese", "mg", 1000),
    "molybdenum_mg": ("Molybdenum", "mg", 1000), "zinc_mg": ("Zinc", "mg", 1000),
    "vita_ug": ("Vitamin A", "ug", 1), "vite_mg": ("Vitamin E", "mg", 1000),
    "vitd2_ug": ("Vitamin D2", "ug", 1), "vitd3_ug": ("Vitamin D3", "ug", 1),
    "vitk1_ug": ("Vitamin K1", "ug", 1), "vitk2_ug": ("Vitamin K2", "ug", 1),
    "folate_ug": ("Folate", "ug", 1), "vitb1_mg": ("Vitamin B1", "mg", 1000),
    "vitb2_mg": ("Vitamin B2", "mg", 1000), "vitb3_mg": ("Vitamin B3", "mg", 1000),
    "vitb5_mg": ("Vitamin B5", "mg", 1000), "vitb6_mg": ("Vitamin B6", "mg", 1000),
    "vitb7_ug": ("Vitamin B7", "ug", 1), "vitb9_ug": ("Vitamin B9", "ug", 1),
    "vitc_mg": ("Vitamin C", "mg", 1000), "carotenoids_ug": ("Carotenoids", "ug", 1)
}
...
def load_and_process_data(filepath="data/food_data.csv"):
    ...
    for _, row in df.iterrows():
        ...
        # --- FIXED: Smart Veg Detection ---
        is_veg = True
        for nv_word in NON_VEG_KEYWORDS:
            # Use Regex to match whole words only
            # Matches "egg" but NOT "veggie" or "eggplant"
            if re.search(r'\b' + re.escape(nv_word) + r'\b', food_name_lower):
                is_veg = False
                break
        ...
        for col, (disp_name, unit, multiplier) in MICRONUTRIENTS_MAP.items():
            serving_col = "unit_serving_" + col
            val = clean_numeric(row.get(serving_col, 0))
            if val == 0: val = clean_numeric(row.get(col, 0))
        ...
        all_micros.sort(key=lambda x: x[0], reverse=True)
        top_5_micros = [f"{name}: {val:.2f}{unit}" for _, val, name, unit in all_micros[:5]]
